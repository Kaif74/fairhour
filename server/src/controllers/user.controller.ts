...
export async function getTimeBalance(
    ...
    try {
        ...
        // Calculate hours provided (earned)
        // Includes self-referential exchanges (signup bonuses)
        const providedExchanges = await prisma.exchange.aggregate({
            where: {
                providerId: userId,
                status: 'COMPLETED',
            },
        ...
        // Calculate hours received (spent)
        // Excludes self-referential exchanges (where user is both provider and requester)
        // This ensures signup bonuses count as earned but not spent
        const receivedExchanges = await prisma.exchange.aggregate({
            where: {
                requesterId: userId,
                status: 'COMPLETED',
                NOT: {
                    providerId: userId, // Exclude self-referential (signup bonus)
                },
            },
            _sum: {
                hours: true,
        ...
        const hoursEarned = providedExchanges._sum.hours || 0;
        const hoursSpent = receivedExchanges._sum.hours || 0;
        const balance = hoursEarned - hoursSpent;
